...
{
  double current_departure =  _flow_queue.empty() ? s_flow.get_departure() :  _flow_queue.back().get_departure();
  Flow new_flow( s_flow );
  new_flow.set_departure( current_departure + s_flow.get_remaining_flow_size() / _link_speed ); 
